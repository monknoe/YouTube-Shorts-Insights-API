...
@app.get("/")
def read_root():
    return {"day": 6, "message": "Welcome to Day 6!"}
...
@app.get("/filter_description")
def filter_description(keyword: Optional[str] = None):
    if keyword is None:
        return items
    else:
        keyword_lower = keyword.lower()
        filtered_items = [
            ...
            if keyword_lower in (item.description or "").lower()
            #因為description可能為None 會導致AttributeError: 'NoneType' object has no attribute 'lower'
        ]
        return filtered_items